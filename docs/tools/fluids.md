...
- `aquifer_layers[]`, `water_layers[]`, `flood_risk_tiles[]`, and `wells[]` are all capped (50 / 200 / 50 / 20 respectively); each has a `_total` and `_truncated` pair to detect overflow. None of the caps were hit on the fixture (6 wells, 9 water layers, 1 aquifer layer), so truncation itself is untested live — only its arithmetic is (covered by the invariant above). `flood_risk_total` is the true count of every qualifying tile found — never capped at an internal collection limit — and `flood_risk_tiles[]` is genuinely the global first 50 by the documented sort order, not just the first 50 encountered mid-scan: since the scan already walks z descending (the sort's own primary key), each z-level's candidates are batched, sorted, and merged into the running top-50 before moving to the next (lower) z; candidate collection stops for good only once that running set already holds 50 entries no lower z could ever outrank, while the true-total count keeps accumulating regardless.
