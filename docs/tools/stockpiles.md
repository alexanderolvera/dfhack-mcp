...
- `item_count`/backlog/rotting all derive from one pass over `df.global.world.items.other.IN_PLAY`, positioned via `dfhack.items.getPosition(item)` and matched against each pile's `(x1..x2, y1..y2, z)` bounding box — confirmed to resolve items sitting inside a container (e.g. a barrel parked on a pile tile), not just loose on-ground items: a 9-tile food pile on the fixture reported `item_count=842`, and inspecting it directly showed 808 individually-tracked `SEEDS` items plus barrels/armor/clothing, all positionally inside the pile's bounds.
...
**Still a deliberate approximation, not a bug**: `fullness_pct`'s `4`-items-per-tile constant reads *far* over 100% on both container-heavy piles (barrels/bins can hold many items per tile — the 9-tile seed pile above reads `fullness_pct=2339`) and single-tile refuse/corpse piles that accumulate for the fort's whole lifetime (the fixture's 1-tile corpse/refuse pile held 113 items and reads `fullness_pct=2825`). Both are real, correctly-counted `item_count` values, not double-counting or a parsing bug — verified by hand against the raw item list in each case. The heuristic was chosen for its extreme cheapness (`O(1)` per pile vs. walking real per-tile stacking/size rules) and is documented above as directional only; live testing didn't surface a case worth retuning the constant for, since even a larger constant would still blow past 100% on a decades-old single-tile dump.
