...
- `routes[]` capped at 100; stops/stockpiles/conditions within a route are not separately capped (bounded in practice by how many a player manually places). `vehicles[]` is capped at 200 **unconditionally only for unreferenced vehicles** — every vehicle a (capped) route actually references (via `routes[].vehicles[].vehicle_id` or a stop's `parked_vehicle_id`) is always kept, even past 200, so `hauling_routes_cross_references_resolve` can never fail on a valid fort from a dropped reference. `vehicles_total`/`vehicles_truncated` still describe the true fort-wide count and whether anything (necessarily unreferenced) was dropped. In the pathological case of a fort whose capped routes alone reference more than 200 vehicles, `vehicles.length` can exceed 200 — an edge case the Dreamfort fixture (9 routes, 9 vehicles) doesn't exercise, but referential integrity was chosen over a hard cap ceiling.
