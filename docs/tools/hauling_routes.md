...
- `routes[]` capped at 100, top-level `vehicles[]` capped at 200; stops/stockpiles/conditions within a route are not separately capped (bounded in practice by how many a player manually places). The `vehicles[]` cap prioritizes every vehicle a (capped) route actually references — via `routes[].vehicles[].vehicle_id` or a stop's `parked_vehicle_id` — over unreferenced ones, so `hauling_routes_cross_references_resolve` can't fail on an otherwise-valid fort just because a referenced vehicle's id happened to sort past the cap on plain ascending order. This still isn't unconditionally safe: a fort with more than 200 vehicles referenced by its (capped) routes would still drop some — an edge case the Dreamfort fixture (9 routes, 9 vehicles) doesn't exercise.
