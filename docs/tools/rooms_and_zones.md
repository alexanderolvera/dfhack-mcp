...
- Active ghosts are units with `unit.flags3.ghostly` set. `unquiet_dead_count` scans `world.history.figures` for this civ/race's dead (`died_year ~= -1`) with `flags.ghost` set, excluding any already counted in `active[]` via their `hist_figure_id` — the two facts never double-count the same ghost.
