...
- `by_group[]` / `by_group_total` / `by_group_truncated` — `{ species, sex, adult, count }`, one row per distinct species/sex/adult-or-child combination, capped at 100 distinct combinations.
- `grazers` — `{ total, pastured, unpastured[], unpastured_truncated }`. `unpastured[]` lists the actual animals (capped 50) — small enough in practice to be actionable, and urgent (a grazer literally cannot eat without a pasture). NOT gated on adulthood — a juvenile grazes too.
- `egg_layers` — `{ total, nestbox_count, pastured_without_nestbox, unpastured }`. Counts only, not a list — the population is usually large (dozens of birds) and the consequence (missed eggs) is mild, unlike grazer starvation. Gated on adulthood — a juvenile of an egg-laying caste cannot actually lay yet, so it's excluded from every figure here.
...
- `cages[]` / `cages_truncated` — `{ building_id, occupants[], occupants_total, occupants_truncated }`, one entry per currently-occupied cage (empty cages are omitted), capped 50 cages. Each cage's OWN `occupants[]` is independently capped at 20 (`occupants_total`/`occupants_truncated` track that cage's real count) — a single densely-packed cage trap can't inflate the response either.
...
- Caps (50 each, with `*_truncated` flags) apply to `grazers.unpastured`, `marked_for_slaughter`, `trained`, and `cages`; each cage's own `occupants[]` additionally caps at 20; `by_group[]` caps at 100 distinct combinations — a large or old fort (or a heavily-modded creature roster, or one densely-packed cage trap) can exceed any of these; the scalar counts are never capped. The fixture had zero occupied cages, so the cage/occupant cap path is verified by code inspection and the same `cap()` helper already proven on the other lists in this file, not by a live positive-case example.
