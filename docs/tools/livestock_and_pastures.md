...
- Caps (50 each, with `*_truncated` flags) apply to `grazers.unpastured`, `marked_for_slaughter`, `trained`, and `cages`; each cage's own `occupants[]` additionally caps at 20 — a large or old fort (or one densely-packed cage trap) can exceed these; `by_group[]` and the scalar counts are never capped. The fixture had zero occupied cages, so the cage/occupant cap path is verified by code inspection and the same `cap()` helper already proven on the other lists in this file, not by a live positive-case example.
