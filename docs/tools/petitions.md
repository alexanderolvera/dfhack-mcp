...
- Lists capped at 50 rows each (`*_truncated` flags); `alerts[]` is uncapped but bounded by the same 50+50 rows it's built from. Each list sorts active rows first (`status: "outstanding"` or `awaiting_decision: true`), then resolved rows, both by ascending `agreement_id` within their group — so on a long-lived fort with more than 50 petitions in a tier, it's the current/pending ones that survive the cap, never old resolved history crowding them out. `awaiting_decision_count` is computed from the full, pre-cap list, so it stays the true total even when a list is truncated. The fixture's 2 rows are far under the cap, so the cap/truncation/prioritization logic itself is untested live (shape-verified by the `petitions_wellformed` invariant, not exercised against 51+ real rows).
